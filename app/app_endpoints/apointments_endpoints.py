...
from app.dependencies import Database,  CurrentUser, Patient, Institution, Clinician, Apointment
...
router = APIRouter(
    prefix="/apointments", tags=["apointments"], responses={404: {"description": "Not Found"}}
)



@router.post("/{apointment_code}", status_code=status.HTTP_201_CREATED, response_model=None)
def post_apointments(
    ...
    if existing_apointment:
        raise HTTPException(
            409, detail="There is already an apointment with this code"
        )

    if not existing_patient:
        raise HTTPException(
            409, detail="There is no patient with this credentials"
        )
    if not existing_clinician:
        raise HTTPException(
            409, detail="There is no clinician with this credentials"
        )
    if not existing_institution:
        raise HTTPException(
            409, detail="There is no institution with this credentials"
        )
