...
from app.dependencies import (ClinicianById, Database, MedicationById,
                              MedicationRequestById)
...
def post_medication_request(
    ...
    if not existing_patient:
        raise HTTPException(404, detail="There is no patient to prescribe")

    if not existing_clinician:
        raise HTTPException(
            404,
            detail="There is no clinician in the system, please ask the admin to add one!",
        )
    ...
    return (
        database.execute(
            ...
            .join(
                ...
                == models.MedicationRequest.medication_reference,
            )
            .join(
                models.Clinician,
                models.Clinician.registration_id
...
def modify_medication_request(
    ...
    if not existing_medication_request:
        raise HTTPException(404, detail="There is no medication request with that id")

    database.execute(
        update(models.MedicationRequest)
        .values(
        ...
        .where(
            ...
            == existing_medication_request.medication_request_id
        )
    )
    database.commit()
