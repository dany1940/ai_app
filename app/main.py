...
from database import db_dependency as get_db
...
def read_items(skip: int = 0, limit: int = 100, db: Session = get_db):
    items = crud.get_items(db, skip=skip, limit=limit)
    return items


@app.post("/transactions/", response_model=schemas.TransactionModel)
async def create_transaction(transaction: schemas.TransactionBase, db: Session = get_db):
